...
const navSections = [
    ...
    },
    {
        title: "Vận hành",
        items: [
            { path: "/admin/courts", label: "Sân", icon: "sports_tennis" },
            { path: "/admin/bookings", label: "Booking", icon: "event_available" },
            { path: "/admin/equipments", label: "Thiết bị", icon: "inventory_2" },
            { path: "/admin/maintenance", label: "Bảo trì", icon: "build" },
        ],
    },
    {
        title: "Quản trị",
        items: [
            ...
            { path: "/admin/coupons", label: "Mã giảm giá", icon: "sell" },
            ...
            >
                ...
                <div className="flex items-center gap-3 border-b border-zinc-200/50 px-6 py-5">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wider">Admin Panel</p>
                    </div>
                    <button
                        onClick={() => setSidebarOpen(false)}
                ...
                <nav className="flex-1 overflow-y-auto px-3 py-4">
                    {navSections.map((section) => (
                        <div key={section.title} className="mb-5 last:mb-0">
                            <p className="mb-2 px-3 text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                                {section.title}
                            </p>
                ...
                <div className="border-t border-zinc-200/50 px-4 py-4">
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-zinc-800 truncate">{user?.fullName || "Admin"}</p>
                    ...
                    <div className="mb-3 grid grid-cols-2 gap-2">
                        {accountLinks.map((item) => (
                            <NavLink
                                ...
                                onClick={() => setSidebarOpen(false)}
                                className={({ isActive }) =>
                                    `flex items-center justify-center gap-1.5 rounded-lg px-2.5 py-2 text-[11px] font-bold transition-all ${
                                        isActive
                                            ? "bg-primary text-white shadow-sm"
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <header className="flex h-16 items-center justify-between border-b border-zinc-200/50 bg-white px-4 shadow-sm lg:px-8">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-xs text-zinc-400 hidden sm:block">Chào mừng trở lại,</span>
                        <span className="text-xs font-bold text-primary">{user?.fullName || "Admin"}</span>
                    </div>
